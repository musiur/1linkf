    ...
    <div className="w-[400px] min-w-[310px] mx-auto grid grid-cols-1 gap-5 section m-5">
      ...
      {formData.file ? (
        <div className="p-3 rounded-md border border-gray-200 grid grid-cols-1 gap-2 mt-10">
          <p className="text-gray-400">You can download your giveway file from here</p>
          <a
          href={formData.file}
          download={`Give-resouces${formData.extension}`}
          target="_blank"
        >
          <button className="px-5 py-1 rounded-md bg-white text-gray-800 hover:shadow-xl border border-[#0891b2] font-semibold w-full">Download the resource</button>
        </a>
        </div>
      ) : null}
    </div>
