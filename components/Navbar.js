      ...
      >
        ...
        {userdata.username ? (
          <li className="flex flex-col gap-5">
            <Link href="/dashboard/profile" className="text-center">
              Profile
            </Link>
            <button onClick={SignOut} className="text-red-400 text-center">
              Sign out
            </button>
          </li>
        ) : (
      ...
      <nav className="bg-[#F9FAFB] relative">
        <div className="flex items-center justify-between gap-5 p-[10px] lg:p-[23px]">
          <div className="flex items-center justify-start gap-[48px]">
            <button
              className="h-[60px] pl-1 pr-2 flex items-center justify-center bg-gradient-to-b from-[#202D3D] to-[#0B91AF] text-[20px] font-bold leading-[28px] text-white cursor-pointer"
              onClick={() => Router.push('/')}
            >
              1link
            </button>
        ...
        {openSubmenu ? (
          <div
            className="absolute top-[80px] right-0 min-w-[200px] border flex flex-col items-end justify-start gap-1 rounded-md bg-white py-1 mr-5"
            onClick={() => setOpenSubmenu(false)}
          >
            <Link
              href="/dashboard/profile"
              className="hover:bg-gray-100 rounded-md w-full text-right cursor-pointer px-3 py-1"
            >
              Profile
            </Link>
            <button
              onClick={SignOut}
              className="hover:bg-red-100 text-red-400 rounded-md w-full text-right cursor-pointer px-3 py-1"
            >
              Sign out
            </button>
          </div>
