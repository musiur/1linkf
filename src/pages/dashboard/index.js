                ...
                <div className="col-span-2 bg-[#0891B2] p-5 min-h-auto lg:min-h-[90vh] h-full flex flex-row flex-wrap lg:flex-col gap-1 justify-start items-start text-white">
                    {
                        tabs.map((item) => {
                            ...
                            return (
                                <Link href={`/dashboard/${tab}`} key={item} className={`w-auto lg:w-full text-center lg:text-left cursor-pointer px-3 py-1 rounded-md ${Router.pathname.includes(tab) ? "bg-[#ffffff50] hover:bg-[#ffffff30]" : "hover:bg-[#ffffff20]"}`}>{item}</Link>
                            )
                        })
                    }
                </div>
