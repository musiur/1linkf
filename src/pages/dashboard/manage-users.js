    ...
    return (
        <Dashboard>
            <div className="relative p-5 h-full">
                                ...
                                return (
                                    <div key={item._id} className="border rounded-md p-5 hover:border-[#0991b2] bg-white">
                                        ...
                                        <div className="flex items-center justify-start gap-1 mt-4">
                                            <Button onClick={() => { setSelected(item); setEdit(true) }}>Update</Button>
                                            <Button type="warning" onClick={() => { setSelected(item); setDeleteModal(true) }}>Delete</Button>
                                        </div>
                                    </div>
                                )
                            })
                ...
                {
                    deleteModal ? <div className="absolute top-0 left-0 w-full h-full bg-[#00000050] flex items-start justify-center pt-10">
...
const UpdateForm = ({ userdata }) => {
    const [formData, setFormData] = useState({ username: userdata.username, email: userdata.email, link: "" });
    ...
    const UpdateApiCall = async () => {
        try {
            setSpinner(true)
            const response = await axios.put(`${process.env.API_HOST}/api/test/user/update/${userdata._id}`, formData, {
                headers: {
                    "x-access-token": sessionStorage.getItem("access_token")
                }
            });
    ...
    return (
        <div>
            ...
            <form id="sign_up_form">
                ...
                <input type="text" name="username" placeholder="Username" onChange={handleOnChange} id="username" defaultValue={formData.username} />
                {
                    errorMessage?.username ? <span>{errorMessage.username}</span> : null
                }
                <label htmlFor="email">Email</label>
                <input type="email" name="email" placeholder="Email" onChange={handleOnChange} id="email" defaultValue={formData.email} />
                {
                    errorMessage?.email ? <span>{errorMessage.email}</span> : null
                }
                <Button onClick={(e) => { setClicked(true); handleOnSubmit(e) }} disable={spinner}>
                    {
            ...
            <div className="mt-5 flex justify-center">
                <Button type="warning" onClick={() => userdata.setEdit(false)}>Close</Button>
            </div>
        </div>
    )
}
...
const DeleteForm = ({ userdata }) => {
    ...
    const DeleteApiCall = async () => {
        try {
            ...
            const response = await axios.delete(`${process.env.API_HOST}/api/test/user/delete/${formData.username}/${userdata._id}`, {
                headers: {
                    "x-access-token": sessionStorage.getItem("access_token")
                }
            });
    ...
    return (
        <div>
            ...
            <form id="sign_up_form">
                <label htmlFor="username" className="break-all">Please enter your username <span className="text-red-400 font-medium break-all">{userdata.username}</span> here</label>
                <input type="text" name="username" placeholder="Username" onChange={handleOnChange} id="username" />
                {
                    errorMessage?.username ? <span>{errorMessage.username}</span> : null
                }
                <div className="my-2 flex justify-center">
                    <Button onClick={(e) => { e.preventDefault(); userdata.setDeleteModal(false) }}>Close</Button>
                </div>
                <Button type="warning-solid" onClick={(e) => { setClicked(true); handleOnSubmit(e) }} disable={spinner}>
                    {
                        spinner ? <Spinner /> : null
                    }
                    {
